...
router.put('/:id/rol', auth, adminOnly, async (req, res) => {
  try {
    ...
    await q('UPDATE usuarios SET rol=? WHERE id=?', [rol, req.params.id]);
...
router.put('/:id/activo', auth, adminOnly, async (req, res) => {
  try {
    const result = await q('SELECT activo FROM usuarios WHERE id=?', [req.params.id]);
    const u = result.rows[0];
    if (!u) return res.status(404).json({ error: 'No encontrado' });
    await q('UPDATE usuarios SET activo=? WHERE id=?', [u.activo ? 0 : 1, req.params.id]);
...
router.get('/notificaciones', auth, async (req, res) => {
  try {
    const result = await q('SELECT * FROM notificaciones WHERE usuario_id=? ORDER BY created_at DESC LIMIT 20', [req.user.id]);
    res.json(result.rows);
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
router.put('/notificaciones/leer', auth, async (req, res) => {
  try {
    await q('UPDATE notificaciones SET leida=1 WHERE usuario_id=?', [req.user.id]);
    res.json({ ok: true });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
router.get('/favoritos', auth, async (req, res) => {
  try {
    const result = await q(`
      ...
      LEFT JOIN categorias c ON p.categoria_id = c.id
      WHERE f.usuario_id = ? ORDER BY f.created_at DESC
    `, [req.user.id]);
    res.json(result.rows);
  } catch (err) { res.status(500).json({ error: err.message }); }
});
